...
def main() -> None:
    ...
    flow.append(Paragraph(
        "log(λ_home) = α + att[h] − def[a] + γ · (1 − is_neutral)<br/>"
    ...
    flow.append(Paragraph(
        ...
        "0.3 · squad-strength composite. ρ ∈ (−0.15, 0.15) controls low-score correlation. "
        "Centered hierarchy with HalfNormal scale priors. NUTS sampling, 4 chains × 2k draws, "
        "0 divergences in production fits.", body))
    flow.append(Paragraph(
        "<b>Match weighting:</b> per-match weight = exp(−ln(2) · age / 2.5yr) · importance_weight, "
        "where importance follows Elo K-factor convention (WC = 1.0, qualifier = 0.65, friendly = 0.30).",
        body))
    flow.append(Paragraph(
